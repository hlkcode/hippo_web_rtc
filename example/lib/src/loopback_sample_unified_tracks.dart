...
class _MyAppState extends State<LoopBackSampleUnifiedTracks> {
  ...
  List<MediaDeviceInfo>? _cameraList;
  Timer? _timer;
  ...
  Future<void> _refreshMediaDevices() async {
    var devices = await navigator.mediaDevices.enumerateDevices();
    _cameraList = await Helper.cameras;
    print('ondevicechange _cameraList = ${_cameraList?.length}');
    print('ondevicechange _cameraList = ${_cameraList}');
    setState(() {
      _mediaDevicesList = devices;
    });
  }
  ...
  void doZoomIn() async {
    ...
    final isZoomable = await track.isZoomSupported();
    print('ISZOOM = $isZoomable');
    // await track.setZoom(++zoom);
    zoom = await track.setZoom1();
    print('+++++ ZOOM = $zoom');
    print('MaxZOOM = ${await track.getMaxZoom()}');
    setState(() {});
  }

  void doZoomOut() async {
    final track = _localStream!.getVideoTracks().first;
    final isZoomable = await track.isZoomSupported();
    print('ISZOOM = $isZoomable');
    // await track.setZoom(--zoom);
    zoom = await track.setZoom5();
    print('+++++ ZOOM = $zoom');
    print('MaxZOOM = ${await track.getMaxZoom()}');
    setState(() {});
  }
  ...
  void testNative() async {
    try {
      final track = _localStream!.getVideoTracks().first;
      await track.setLightOn();
      await track.setLightOff();
    } catch (e) {
      print(e);
    }
  }
